...
ENV CRONTAB_15MIN='*/15 * * * *' \
    ...
    JOB_300_WHAT='backup' \
    JOB_300_WHEN='daily' \
    JOB_500_WHAT='backup && dup remove-all-but-n-full 2 --force $DST && dup cleanup --force $DST' \
    JOB_500_WHEN='weekly' \
    OPTIONS='' \
    OPTIONS_EXTRA='--metadata-sync-mode partial --full-if-older-than 1M --file-prefix-archive archive-$(hostname -f)- --file-prefix-manifest manifest-$(hostname -f)- --file-prefix-signature signature-$(hostname -f)- --s3-use-deep-archive' \
    SMTP_HOST='smtp' \
    SMTP_PASS='' \
    SMTP_PORT='25' \
    SMTP_TLS='' \
